...
if [[ $# -eq 0 ]] ; then
    ./fast-downward.py --build="$BUILD" multi-output.sas --search "multiple()"
elif [[ "$1" == "build" ]] ; then
    ./build.py -j 6 "$BUILD"
elif [[ "$1" == "memory" ]] ; then
    valgrind --leak-check=yes ./builds/$BUILD/bin/downward --search "multiple()" --internal-plan-file sas_plan < multi-output.sas > /dev/null 2>output.log
    gedit output.log
    rm output.log
elif [[ "$1" == "profile" ]] ; then
    valgrind --tool=callgrind --dump-instr=yes --collect-jumps=yes ./builds/$BUILD/bin/downward --search "multiple()" --internal-plan-file sas_plan < multi-output.sas > /dev/null
