...
if [[ $# -eq 0 ]] ; then
    ./fast-downward.py multi-output.sas --search "multiple(lazy_greedy(ff()))"
elif [[ "$1" == "build" ]] ; then
    ./build.py -j 6 release64
elif [[ "$1" == "memory" ]] ; then
    valgrind --leak-check=yes ./builds/release64/bin/downward --search "multiple(lazy_greedy(ff()))" --internal-plan-file sas_plan < multi-output.sas > /dev/null 2>output.log
    gedit output.log
    rm output.log
elif [[ "$1" == "profile" ]] ; then
